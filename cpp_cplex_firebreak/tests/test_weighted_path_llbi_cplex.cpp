...
firebreak::opt::OptimizationInstance make_weighted_instance(bool homogeneous = false) {
    ...
    opt.landscape_name = "weighted_path_llbi_cplex";
    opt.alpha = 0.2;
    opt.n_cells = 5;
    opt.budget = 1;
...
firebreak::solver::ModelResult solve_callback(
    ...
    options.strengthening_options.use_path_llbi = path;
...
void assert_path_diagnostics(const firebreak::solver::ModelResult& result) {
    assert(result.path_llbi_enabled);
    assert(result.path_llbi_weighted);
    assert(!result.path_llbi_weight_map_hash.empty());
    assert(result.path_llbi_scenarios_precomputed > 0);
    assert(result.path_llbi_baseline_nodes > 0);
    assert(result.path_llbi_auxiliary_variables == result.path_llbi_num_b_vars);
    assert(result.path_llbi_path_constraints == result.path_llbi_num_path_constraints);
    assert(result.path_llbi_total_paths == result.path_llbi_num_paths_used);
    assert(result.path_llbi_total_candidate_incidence_terms > 0);
    assert(result.path_llbi_validity_mode == "directed-simple-path-burning-lower-bound");
}
...
void compare_callback_variants(
    ...
    const auto baseline = solve_callback(opt, risk_config, false, false, false, false);
    const auto path = solve_callback(opt, risk_config, true, false, false, false);
    const auto standard_path = solve_callback(opt, risk_config, true, false, true, false);
    const auto coverage_path = solve_callback(opt, risk_config, true, true, false, false);
    const auto root_path = solve_callback(opt, risk_config, true, false, false, true);
    const auto explicit_path = solve_explicit(opt, risk_config);

    assert(baseline.status == "Optimal");
    assert(path.status == "Optimal");
    assert(standard_path.status == "Optimal");
    assert(coverage_path.status == "Optimal");
    assert(root_path.status == "Optimal");
    assert(explicit_path.status == "Optimal");
    assert_close(baseline.objective_value, direct.objective_value);
    assert_close(path.objective_value, direct.objective_value);
    assert_close(standard_path.objective_value, direct.objective_value);
    assert_close(coverage_path.objective_value, direct.objective_value);
    assert_close(root_path.objective_value, direct.objective_value);
    assert_close(explicit_path.objective_value, direct.objective_value);
    assert_path_diagnostics(path);
    assert_path_diagnostics(standard_path);
    assert_path_diagnostics(coverage_path);
    assert_path_diagnostics(root_path);
    assert_path_diagnostics(explicit_path);
}
...
void test_homogeneous_regression() {
    ...
    const auto implicit_result =
        solve_callback(implicit, firebreak::risk::RiskMeasureConfig(), true, false, false, false);
    const auto explicit_result =
        solve_callback(explicit_unit, firebreak::risk::RiskMeasureConfig(), true, false, false, false);
    assert_close(implicit_result.objective_value, explicit_result.objective_value);
    assert(implicit_result.path_llbi_num_b_vars == explicit_result.path_llbi_num_b_vars);
    assert(implicit_result.path_llbi_num_path_constraints ==
           explicit_result.path_llbi_num_path_constraints);
    assert(!implicit_result.path_llbi_weighted);
    assert(!explicit_result.path_llbi_weighted);
}
...
int main() {
    ...
    std::cout << "Skipping weighted PathLLBI CPLEX tests because CPLEX is not enabled.\n";
    return 0;
#else
    test_expected_cvar_mean_cvar_equivalence();
    test_homogeneous_regression();
    std::cout << "All weighted PathLLBI CPLEX tests passed.\n";
    return 0;
#endif
}
