...
std::filesystem::path default_experiment_csv_path() {
    return firebreak::io::resolve_output_path("results/experiments/dpv_benders_oos_results.csv");
}
...
void write_benders_cut_export_csv(
    ...
    out << "iteration,scenario_id,subproblem_value,violation,rhs_constant,num_nonzero_coeffs,coefficients\n";
...
void write_lifted_lower_bound_export_csv(
    ...
    std::ofstream out(output_path);
    if (!out) {
        throw std::runtime_error("Could not open lifted lower-bound export CSV file: " + output_path.string());
    }

    out << "scenario_id,f_empty,num_nonzero_coeffs,rhs_constant,coefficients\n";
    for (const auto& inequality : inequalities) {
        ...
        for (const auto& [compact_index, coefficient] : inequality.coefficients_by_compact_index) {
            if (std::fabs(coefficient) <= kCoefficientThreshold) {
                continue;
            }
            if (!first) {
                coefficients << ";";
            }
            coefficients << opt.node_mapper.to_node(compact_index)
                         << ":" << format_cut_number(coefficient);
            first = false;
            ++nonzero;
        }
...
int DpvBendersOutOfSampleRunner::run(const DpvBendersOutOfSampleOptions& options) const {
    ...
    benders::DpvBendersOptions solver_options;
    ...
    if (!benders_cut_export_path.empty()) {
        write_benders_cut_export_csv(benders_cut_export_path, solve_result.benders_added_cuts);
    }
    ...
    solution_record.method = "DPV-SAA-Benders";
    ...
    result.notes.push_back(
        std::string("Benders lifted lower bounds enabled: ") +
    ...
    if (!lifted_lower_bound_export_path.empty()) {
        result.notes.push_back(
            "Lifted lower-bound inequalities were exported to " +
