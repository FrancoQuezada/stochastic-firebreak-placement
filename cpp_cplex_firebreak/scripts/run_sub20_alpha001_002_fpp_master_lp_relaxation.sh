...
RESULTS_DIR="results/diagnostics/sub20_alpha001_002_fpp_master_lp_relaxation"
EXPERIMENT_ID="sub20_alpha001_002_fpp_master_lp_relaxation"
LANDSCAPE="Sub20"
...
for arg in "$@"; do
  case "${arg}" in
    --resume)
      ...
      ;;
    --help|-h)
      echo "Usage: bash scripts/run_sub20_alpha001_002_fpp_master_lp_relaxation.sh [--resume]"
      exit 0
      ;;
    *)
...
python3 scripts/validate_sub20_alpha001_002_fpp_master_lp_relaxation.py \
  --results-dir "${RESULTS_DIR}" \
  --expected-rows 50 \
  --seed-base "${SEED_BASE}"

python3 scripts/analyze_sub20_alpha001_002_fpp_master_lp_relaxation.py \
  --results-dir "${RESULTS_DIR}"

echo "Diagnostic complete. CSV: ${OUTPUT_CSV}"
