...
DEFAULT_RESULTS_DIR = "results/batch/fpp_projected_llbi_scaling"


def is_missing(value):
    return value is None or value == "" or value.lower() in {"nan", "na", "none"}
...
def projected_variant(row):
    method = row.get("method", "")
    value = row.get("projected_llbi_strategy", "").strip().lower()
...
def summarize(rows, keys):
    ...
    for key_values, group in sorted(groups.items()):
        ...
        out["status_counts"] = ";".join(f"{k}:{v}" for k, v in sorted(Counter(row.get("solver_status", "") for row in group).items()))
...
def validate_splits(rows):
    ...
    if ok:
        report.append(f"PASS controlled split validation for {len(groups)} (landscape, alpha, train_count, case_id) blocks.")
    ...
    if ok:
        report.append(f"PASS cross-alpha split reuse validation for {len(cross_alpha)} (landscape, train_count, case_id) blocks.")
    return ok, report
    ...
    for key in sorted({k for (k, _fam, _var) in indexed}):
        for label, lhs_key, rhs_key in pairs:
            ...
            if lhs is None or rhs is None:
                continue
            objective, alpha, train, case_id = key
            out.append(comparison_row({
                "comparison": label,
...
def main():
    ...
    ok, validation_lines = validate_splits(rows)
    expected_methods = 24
    block_counts = Counter(logical_key(row) for row in rows)
    incomplete = {k: v for k, v in block_counts.items() if v not in {expected_methods}}
    if incomplete:
        validation_lines.append(
            "WARN some controlled blocks do not have 24 rows: " +
    ...
    summary_fields = [
        "objective_family", "alpha", "train_scenario_count", "method_family", "method",
        "projected_llbi_family_reconstructed", "projected_llbi_variant_reconstructed",
        "runs", "solved_to_optimality", "time_limited_with_incumbent",
        "failed_or_without_incumbent", "status_counts",
    ] + [name for name, _ in SUMMARY_METRICS] + ["avg_combinatorial_cut_count"]

    write_csv(
        os.path.join(output_dir, "summary_by_objective_alpha_traincount_method.csv"),
        summarize(rows, [
            "objective_family", "alpha", "train_scenario_count", "method_family", "method",
            "projected_llbi_family_reconstructed", "projected_llbi_variant_reconstructed",
        ]),
    ...
    write_csv(
        os.path.join(output_dir, "summary_by_alpha_traincount_method.csv"),
        summarize(rows, [
            "alpha", "train_scenario_count", "method_family", "method",
            "projected_llbi_family_reconstructed", "projected_llbi_variant_reconstructed",
        ]),
    )
